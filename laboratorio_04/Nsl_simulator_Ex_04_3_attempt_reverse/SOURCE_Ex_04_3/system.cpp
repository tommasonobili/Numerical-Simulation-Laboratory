...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  ifstream Primes("../INPUT_Ex_04_3/Primes");
  Primes >> p1 >> p2 ;
  Primes.close();
  int seed[4]; // Read the seed of the RNG
  ifstream Seed("../INPUT_Ex_04_3/seed.in");
  Seed >> seed[0] >> seed[1] >> seed[2] >> seed[3];
  _rnd.SetRandom(seed,p1,p2);

  ofstream couta("../OUTPUT_Ex_04_3/acceptance.dat"); // Set the heading line in file ../OUTPUT_Ex_04_3/acceptance.dat
  couta << "#   N_BLOCK:  ACCEPTANCE:" << endl;
  couta.close();

  ifstream input("../INPUT_Ex_04_3/input.dat"); // Start reading ../INPUT_Ex_04_3/input.dat
  ofstream coutf;
  coutf.open("../OUTPUT_Ex_04_3/output.dat");
  string property;
  double delta;
  while ( !input.eof() ){
    input >> property;
...
// modificato in eserecizio 4.2
void System :: initialize_velocities(){
  double xold, yold, zold;
    if(_restart){ //con restart=1
        ifstream vin("../INPUT_Ex_04_3/CONFIG/velocities.out");
        double vx, vy, vz;

        if(vin.is_open()){
            for(int i=0; i<_npart; i++){
                vin >> vx >> vy >> vz;
                _particle(i).setvelocity(0, -vx);   // il "-" perchè è come andare a ritroso
                _particle(i).setvelocity(1, -vy);
                _particle(i).setvelocity(2, -vz);
            }
        } else {
            cerr << "PROBLEM: Unable to open INPUT file velocities.out" << endl;
            exit(EXIT_FAILURE);
        }
        vin.close();
    ...
    //inzializzo una sola componente della velocità per ogni particella uguale a v_0
...
void System :: initialize_properties(){ // Initialize data members used for measurement of properties
  ...
  ifstream input("../INPUT_Ex_04_3/properties.dat");
  if (input.is_open()){
    while ( !input.eof() ){
      input >> property;
      if( property == "POTENTIAL_ENERGY" ){
        ofstream coutp("../OUTPUT_Ex_04_3/potential_energy.dat");
        ...
        ofstream coutk("../OUTPUT_Ex_04_3/kinetic_energy.dat");
        ...
        ofstream coutt("../OUTPUT_Ex_04_3/total_energy.dat");
        ...
        ofstream coutte("../OUTPUT_Ex_04_3/temperature.dat");
        ...
        ofstream coutpr("../OUTPUT_Ex_04_3/pressure.dat");
        ...
        ofstream coutgr("../OUTPUT_Ex_04_3/gofr.dat");
        ...
        ofstream coutpv("../OUTPUT_Ex_04_3/pofv.dat");
        ...
        ofstream coutpr("../OUTPUT_Ex_04_3/magnetization.dat");
        ...
        ofstream coutpr("../OUTPUT_Ex_04_3/specific_heat.dat");
        ...
        ofstream coutpr("../OUTPUT_Ex_04_3/susceptibility.dat");
        ...
        ofstream coutf;
        coutf.open("../OUTPUT_Ex_04_3/output.dat",ios::app);
...
void System :: write_configuration(){
  ofstream coutf;
  if(_sim_type < 2){
    coutf.open("../OUTPUT_Ex_04_3/CONFIG/config.xyz");
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i=0; i<_npart; i++){
    ...
    coutf.open("../OUTPUT_Ex_04_3/CONFIG/conf-1.xyz");
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i=0; i<_npart; i++){
  ...
  coutf.open("../OUTPUT_Ex_04_3/CONFIG/velocities.out"); //salvo velocità finali
  if(coutf.is_open()){
      for(int i=0; i<_npart; i++){
    ...
    coutf.open("../OUTPUT_Ex_04_3/CONFIG/config.spin");
...
void System :: write_XYZ(int nconf){
  ofstream coutf;
  coutf.open("../OUTPUT_Ex_04_3/CONFIG/config_" + to_string(nconf) + ".xyz");
  if(coutf.is_open()){
    coutf << _npart << endl;
    coutf << "#Comment!" << endl;
    for(int i=0; i<_npart; i++){
...
void System :: read_configuration(){
    ...
    if(!_restart){
        cinf.open("../INPUT_Ex_04_3/CONFIG/config.fcc");
        if(cinf.is_open()){
            cinf >> ncoord;
            if (ncoord != _npart){
          ...
          cinf.open("../INPUT_Ex_04_3/CONFIG/config.xyz");
          if(cinf.is_open()){
              cinf >> ncoord;
              if (ncoord != _npart){
  ...
  if(_restart and _sim_type > 1){
    int spin;
    cinf.open("../INPUT_Ex_04_3/CONFIG/config.spin");
    for(int i=0; i<_npart; i++){
      cinf >> spin;
...
void System :: averages(int blk){
  ...
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
    coutf.open("../OUTPUT_Ex_04_3/potential_energy.dat",ios::app);
  ...
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy){
    coutf.open("../OUTPUT_Ex_04_3/kinetic_energy.dat",ios::app);
  ...
  // TOTAL ENERGY //////////////////////////////////////////////////////////////
  if (_measure_tenergy){
    coutf.open("../OUTPUT_Ex_04_3/total_energy.dat",ios::app);
  ...
  // TEMPERATURE ///////////////////////////////////////////////////////////////
  if (_measure_temp){
    coutf.open("../OUTPUT_Ex_04_3/temperature.dat",ios::app);
  ...
  if (_measure_pofv){
    coutf.open("../OUTPUT_Ex_04_3/pofv.dat",ios::app);
  ...
  // ACCEPTANCE ////////////////////////////////////////////////////////////////
  double fraction;
  coutf.open("../OUTPUT_Ex_04_3/acceptance.dat",ios::app);
