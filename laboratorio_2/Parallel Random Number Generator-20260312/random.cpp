...
double Random :: cos_importance_sampling(){
   // This function generates a random number from a distribution proportional to cos(pi*x/2) using the inverse transform method
   //function chosen for importance sampling is p(x)=2*(1-x) ----> x=1-sqrt(1-y) where y is a uniform random number in [0,1)
   double y=Rannyu();
   return 1-sqrt(1-y);
}
