...
void block_swap_mutation(Random &rnd, Individual &individual) {
    ...
    // Uso al massimo tail_size / 3 per evitare casi difficili con blocchi troppo grandi.
...
void inversion_mutation(Random &rnd, Individual &individual) {
    ...
    // Lunghezza del blocco da invertire: almeno 2 città
    int block_length = 2 + int(rnd.Rannyu(0.0, double(tail_size - 1)));

    // Posizione iniziale del blocco nella tail, cioè escludendo la città 1
    int start = int(rnd.Rannyu(0.0, double(tail_size - block_length + 1)));
...
int main() {
    ...
    const string cities_filename = "cities_square.txt";

    const int number_of_cities = 34;
    const int population_size = 1000;
    const int number_of_generations = 500;

    const double selection_power = 2.5;
    ...
    for (int generation = 1; generation <= number_of_generations; generation++) {
        ...
        //non sono riuscito ad avere un early stopping efficace, quindi lo tolgo per vedere l'andamento completo dell'evoluzione. Rischiavo di perdermi
        // delle mutazioni casuali che dopo tot tempo di risultati costanti sbloccavano nuovi schemi e ridavano inizio ad un evoluzione efficace. 
    }
